...
const SECTION_STYLES = {
  ...
  tip: { borderLeft: `4px solid ${ACCENT}` },
};
    ...
    <div className="card" style={{ ...style, position: 'relative' }}>
      {label && (
        <p style={{
          fontSize: '0.8125rem',
          fontWeight: 600,
          color: section.content_type === 'tip' ? ACCENT
      ...
      {!section.is_read && (
        <button
          ...
          style={{ marginTop: '0.75rem', fontSize: '0.8125rem', color: ACCENT }}
          ...
          <div className="card">
            ...
            {scoreData.topic_breakdown.map((t) => (
              <div key={t.topic} className="row-between" style={{ padding: '0.375rem 0', borderBottom: '1px solid #2A2A2E' }}>
                ...
                <span style={{ fontSize: '0.875rem', color: t.percent >= 70 ? ACCENT : '#F59E0B' }}>
                  ...
                  {q.options.map((opt) => {
                    ...
                    return (
                      <div key={opt.key} style={{
                        ...
                        borderRadius: 8,
                        border: '1px solid',
                        borderColor: isCorrect ? ACCENT : wasSelected ? '#EF4444' : '#2A2A2E',
                        backgroundColor: isCorrect ? 'rgba(249,115,22,0.1)' : wasSelected ? 'rgba(239,68,68,0.1)' : 'transparent',
                        fontSize: '0.9375rem',
                      }}>
      ...
      <div className="card">
        ...
        <div className="stack-sm">
          {q.options.map((opt) => {
            ...
            let border = '#2A2A2E';
            if (isRevealed) {
              if (opt.key === q.correct_answer) { bg = 'rgba(249,115,22,0.1)'; border = ACCENT; }
              else if (opt.key === selected) { bg = 'rgba(239,68,68,0.1)'; border = '#EF4444'; }
            } else if (opt.key === selected) {
              bg = 'rgba(249,115,22,0.08)'; border = ACCENT;
            }
